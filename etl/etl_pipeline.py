...
def run_etl_pipeline():
    ...
    if config.USE_MOCK:
        ...
        rows = [
            ...
            {"order_id": 102, "user_id": 2, "total_amount": 25.50, "payment_method": "ABA KHQR", "created_at": target_date, "book_id": 15, "quantity": 1, "price": 25.50},
        ]
...
def process_data(rows, target_date):
    ...
    file_name = f"orders_extract_{target_date.replace('-', '')}.jsonl"
    output_file_path = os.path.join(output_dir, file_name)

    with open(output_file_path, 'w', encoding='utf-8') as f:
        f.write(json_data + "\n")

    logging.info(f"ETL pipeline executed successfully. Exported {len(rows)} records locally to: {output_file_path}")
